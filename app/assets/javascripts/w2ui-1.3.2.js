...
var w2utils = (function () {
	...
	function isDate (val, format, retDate) {
		...
		if (dt == 'Data Inválida') return false;
	...
	function formatDate (dateStr, format) { // IMPORTANT dateStr HAS TO BE valid JavaScript Date String
		...
		return format.toLowerCase()
			.replace('mes', w2utils.settings.fullmonths[month])
			.replace('mês', w2utils.settings.shortmonths[month])
			.replace(/yyyy/g, year)
			.replace(/yyy/g, year)
...
(function () {
	var w2grid = function(options) {
		...
		this.last = {
			field		: 'Todos',
			caption		: w2utils.lang('Todos os campos'),
			logic		: 'OR',
			search		: '',
			searchIds 	: [],
			multi		: false,
	...
	w2grid.prototype = {
		...
		buttons: {
			'reload'	: { type: 'button', id: 'reload', img: 'icon-reload', hint: 'Recarregar dados' },
			'columns'	: { type: 'drop', id: 'column-on-off', img: 'icon-columns', hint: 'Mostrar/esconder colunas', arrow: false, html: '' },
			'search'	: { type: 'html',   id: 'search',
							html: '<div class="w2ui-icon icon-search-down w2ui-search-down" title="'+ 'Selecione campo para pesquisar' +'" '+
								  'onclick="var obj = w2ui[$(this).parents(\'div.w2ui-grid\').attr(\'name\')]; obj.searchShowFields(this);"></div>'
						  },
			'search-go'	: { type: 'check',  id: 'search-advanced', caption: '<i class="w2ui-icon icon-search-down" title="Pesquisar"/>', hint: 'Abrir pesquisa por campos' },
			'add'		: { type: 'button', id: 'add', caption: '', hint: 'Adicionar nova entrada', img: 'icon-add' },
			'edit'		: { type: 'button', id: 'edit', caption: '', hint: 'Editar selecionada', img: 'icon-edit', disabled: true },
			'delete'	: { type: 'button', id: 'delete', caption: 'Deletar', hint: 'Deletar selecionada', img: 'icon-delete', disabled: true },
			'save'		: { type: 'button', id: 'save', caption: 'Salvar', hint: 'Salvar modificadas', img: 'icon-save' }
		},
		...
		localSort: function (silent) {
			...
			if (silent !== true) setTimeout(function () { obj.status('Ordenar levou ' + time/1000 + ' sec'); }, 10);
			return time;
		},
		...
		localSearch: function (silent) {
			...
			time = (new Date()).getTime() - time;
			if (silent !== true) setTimeout(function () { obj.status('Pesquisa levou ' + time/1000 + ' seg'); }, 10);
			return time;
		},
		...
		search: function (field, value) {
			...
			if (arguments.length == 0) {
				...
				for (var s in this.searches) {
					...
					if ((value1 != '' && value1 != null) || (typeof value2 != 'undefined' && value2 != '')) {
						...
						try {
							if (search.type == 'date' && operator == 'entre') {
							...
							if (search.type == 'date' && operator == 'é') {
							...
							for (var s in this.searches) {
								...
								if (search.type == 'text' || (search.type == 'int' && w2utils.isInt(value)) || (search.type == 'float' && w2utils.isFloat(value))
									...
									var tmp = {
										field	 : search.field,
										type	 : search.type,
										operator : (search.type == 'text' ? 'contém' : 'é'),
										value	 : value
									};
									searchData.push(tmp);
								}
			...
			if ($.isArray(field)) {
				...
				for (var f in field) {
					...
					if (search == null) search = { type: 'text', operator: 'contém' };
		...
		searchReset: function (noRefresh) {
			...
			if (this.last.multi) {
				if (!this.multiSearch) {
					...
					this.last.field  	= 'all';
					this.last.caption 	= w2utils.lang('Todos os campos');
				}
			}
		...
		refresh: function () {
			...
			if (!this.multiSearch && this.last.field == 'Todos' && this.searches.length > 0) {
				this.last.field 	= this.searches[0].field;
				this.last.caption 	= this.searches[0].caption;
			}
			for (var s in this.searches) {
			...
			if (this.last.multi) {
				searchEl.attr('placeholder', '[' + w2utils.lang('Multiplos campos') + ']');
			} else {
		...
		initColumnOnOff: function () {
			...
			if (url) {
				col_html +=
						...
						'				onchange="w2ui[\''+ obj.name +'\'].columnOnOff(this, event, \'skip\', this.value);"> '+ w2utils.lang('Items')+
						'	</div>'+
						'</td></tr>';
			}
			col_html +=	'<tr><td colspan="2" onclick="w2ui[\''+ obj.name +'\'].columnOnOff(this, event, \'line-numbers\');">'+
						'	<div style="cursor: pointer; padding: 4px 8px; cursor: default">'+ w2utils.lang('Mostrar núm. de linhas') +'</div>'+
						'</td></tr>'+
						'<tr><td colspan="2" onclick="w2ui[\''+ obj.name +'\'].columnOnOff(this, event, \'resize\');">'+
						'	<div style="cursor: pointer; padding: 4px 8px; cursor: default">'+ w2utils.lang('Resetar tam. das Colunas') + '</div>'+
						'</td></tr>';
			col_html += "</table></div>";
			...
			if (typeof this.toolbar['render'] == 'undefined') {
				...
				if (this.show.toolbarSearch) {
					var html =
						...
						'	</td>'+
						'	<td>'+
						'		<div title="'+ w2utils.lang('Limpar pesquisa') +'" class="w2ui-search-clear" id="grid_'+ this.name +'_searchClear"  '+
		...
		initSearches: function () {
			...
			for (var s in this.searches) {
				...
				switch (String(search.type).toLowerCase()) {
					case 'alphaNumeric':
					case 'text':
						$('#grid_'+ this.name +'_operator_'+s).val('começa com');
						break;
		...
		getSearchesHTML: function () {
			...
			for (var i = 0; i < this.searches.length; i++) {
				...
				if (s.type == 'text') {
					var operator =  '<select id="grid_'+ this.name +'_operator_'+ i +'">'+
						'	<option value="é">'+ w2utils.lang('é') +'</option>'+
						'	<option value="começa com">'+ w2utils.lang('começa com') +'</option>'+
						'	<option value="contém">'+ w2utils.lang('contém') +'</option>'+
						'	<option value="termina com">'+ w2utils.lang('termina com') +'</option>'+
						'</select>';
				}
				if (s.type == 'int' || s.type == 'float' || s.type == 'date') {
					var operator =  '<select id="grid_'+ this.name +'_operator_'+ i +'" '+
						...
						'			  if ($(this).val() == \'entre\') { range.show(); fld2.w2field(\'clear\').w2field(\''+ s.type +'\'); }'+
						...
						'	<option value="entre">'+ w2utils.lang('entre') +'</option>'+
						'</select>';
				}
				if (s.type == 'list') {
					var operator =  'is <input type="hidden" value="é" id="grid_'+ this.name +'_operator_'+ i +'">';
				}
				html += '<tr>'+
			...
			html += '<tr>'+
					...
					'		<div>'+
					'		<input type="button" onclick="obj = w2ui[\''+ this.name +'\']; if (obj) { obj.searchReset(); }" value="'+ w2utils.lang('Limpar') + '">'+
					'		<input type="button" onclick="obj = w2ui[\''+ this.name +'\']; if (obj) { obj.search(); }" value="'+ w2utils.lang('Pesquisar') + '">'+
					'		</div>'+
					'	</td>'+
					'</tr></table>';
			return html;
		...
		getRecordHTML: function (ind, lineNum, summary) {
			...
			if (this.show.expandColumn) {
				...
				if (record.expanded == 'spinner') tmp_img = '<div class="w2ui-spinner"  title="Expandir" style="width: 16px; margin: -2px 2px;"></div>';
				rec_html += 
						'<td id="grid_'+ this.name +'_cell_'+ ind +'_expand"  title="Expandir" class="w2ui-grid-data w2ui-col-expand">'+
							(summary !== true ?
		...
		status: function (msg) {
			if (typeof msg != 'undefined') {
				...
				var sel = this.getSelection();
				if (sel.length > 0) {
					msgLeft = String(sel.length).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + ' ' + w2utils.lang('selecionados');
					var tmp = sel[0];
					if (typeof tmp == 'object') tmp = tmp.recid+ ', '+ w2utils.lang('Column') +': '+ tmp.column;
					if (sel.length == 1) msgLeft = w2utils.lang('Selecionado') + ': '+ tmp + ' ';
				}
			...
			function getNodeHTML(nd) {
				...
				if (nd.group) {
					...
					'<div class="w2ui-node '+ (nd.selected ? 'w2ui-selected' : '') +' '+ (nd.disabled ? 'w2ui-disabled' : '') +'" id="node_'+ nd.id +'" style="'+ (nd.hidden ? 'display: none;' : '') +'"'+
						...
						'		if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;">'+ 
						'	<div class="w2ui-expand" title="Expandir">' 	+ (nd.nodes.length > 0 ? (nd.expanded ? '-' : '+') : (nd.plus ? '+' : '')) + '</div>' +
						'</td>'+
	...
	$.extend(w2field, {
				...
				switch (tp) {
					...
					case 'enum':
						...
						var defaults = {
							...
							match 		: 'começa com', 	// ['começa com', 'contém']
						...
						$(this).data('selecionado', settings.selected);
						$(this).css({ 
							'padding'			: '0px',
		...
		list_render: function (search) {
			...
			for (var a in items) {
				...
				if (settings.match.toLowerCase() == 'contém' && txt2.indexOf(txt1) != -1) match = true;
				if (match) {
